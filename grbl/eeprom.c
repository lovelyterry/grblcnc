...
static unsigned char eeprom_buffer[0x400];

#ifndef NOEEPROMSUPPORT
void eeprom_flush() {
    uint32_t  nAddress = EEPROM_START_ADDRESS;
    uint16_t* pBuffer  = (uint16_t*)eeprom_buffer;
...
void eeprom_init() {
    uint16_t VarIdx = 0;
    uint8_t* pTmp   = eeprom_buffer;

    FLASH_Unlock();
    for (VarIdx = 0; VarIdx < PAGE_SIZE; VarIdx++) {
        *pTmp++ = (*(volatile uint8_t*)(EEPROM_START_ADDRESS + VarIdx));
    }
