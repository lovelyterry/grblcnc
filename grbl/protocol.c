        ...
        if (sys.suspend & SUSPEND_HOLD_COMPLETE) {
            ...
            if (sys.state & (STATE_SAFETY_DOOR | STATE_SLEEP)) {
                ...
                if (bit_isfalse(sys.suspend, SUSPEND_RETRACT_COMPLETE)) {
                    ...
                    if ((bit_istrue(settings.flags, BITFLAG_HOMING_ENABLE)) && (parking_target[PARKING_AXIS] < PARKING_TARGET) && bit_isfalse(settings.flags, BITFLAG_LASER_MODE) && (sys.override_ctrl == OVERRIDE_PARKING_MOTION)) {
#else
                    if ((bit_istrue(settings.flags, BITFLAG_HOMING_ENABLE)) && (parking_target[PARKING_AXIS] < PARKING_TARGET) && bit_isfalse(settings.flags, BITFLAG_LASER_MODE)) {
#endif
